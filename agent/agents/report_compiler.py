...
# Brothers Automate defaults - used when brand.dev data is unavailable
BROTHERS_AUTOMATE_THEME: dict[str, str] = {
    "primary": "#1a365d",
...
*Report generated by {brand.brand_name} Intelligence Report*
"""
        return report
...
{data_bundle}""",
    ...
    def _luminance(hex_color: str) -> float:
        """Calculate relative luminance of a hex color for contrast decisions."""
        h = hex_color.lstrip("#")
        if len(h) != 6:
            return 0.5
    ...
    def resolve_theme(self, brand: BrandProfile) -> dict[str, str]:
        """Resolve the theme from brand visual identity, falling back to Brothers Automate defaults."""
        vi = brand.visual_identity
        if not vi or vi.source != "brand_dev" or not vi.colors:
            self.logger.info("Using Brothers Automate fallback theme")
            return dict(BROTHERS_AUTOMATE_THEME)

        # Build theme from brand.dev data
        theme = dict(BROTHERS_AUTOMATE_THEME)  # start with defaults
        theme["source"] = "brand_dev"

        # Map colors by role
        primary = None
        secondary = None
        accent = None
        for color in vi.colors:
            if color.role == "primary" and not primary:
                primary = color.hex
            elif color.role == "secondary" and not secondary:
                secondary = color.hex
            elif color.role == "accent" and not accent:
                accent = color.hex

        # Fallback: if roles aren't assigned, use positional
        if not primary and len(vi.colors) >= 1:
            primary = vi.colors[0].hex
        if not secondary and len(vi.colors) >= 2:
            secondary = vi.colors[1].hex
        if not accent and len(vi.colors) >= 3:
            accent = vi.colors[2].hex

        if primary:
            theme["primary"] = primary
            theme["text_primary"] = primary
            theme["primary_light"] = secondary or self._lighten(primary, 0.2)
            # Determine header text color based on luminance
            theme["header_text_color"] = "#ffffff" if self._luminance(primary) < 0.4 else "#1a1a1a"

        if accent:
            theme["accent"] = accent
            theme["accent_dark"] = self._darken(accent, 0.15)
        elif secondary:
            # Use secondary as accent if no explicit accent
            theme["accent"] = secondary
            theme["accent_dark"] = self._darken(secondary, 0.15)
        elif primary:
            # Derive accent from primary by shifting warmth
            theme["accent"] = self._lighten(primary, 0.3)
            theme["accent_dark"] = self._lighten(primary, 0.15)

        if secondary:
            theme["blue"] = secondary

        # Logo - prefer full logo, then icon
        logo_url = ""
        if vi.logos:
            # Prefer type="logo" over "icon", prefer mode="dark" for light backgrounds
            sorted_logos = sorted(vi.logos, key=lambda l: (
                0 if l.type == "logo" else 1,
                0 if l.mode == "dark" else 1,
            ))
            logo_url = sorted_logos[0].url
        theme["logo_url"] = logo_url

        # Typography
        if vi.typography.heading_font:
            theme["font_heading"] = vi.typography.heading_font
        if vi.typography.body_font:
            theme["font_body"] = vi.typography.body_font

        # Slogan/tagline
        if vi.slogan:
            theme["tagline"] = vi.slogan
        elif brand.tagline:
            theme["tagline"] = brand.tagline

        # Footer
        theme["footer_text"] = f"{brand.brand_name} Intelligence Report"

        self.logger.info(
            "Resolved theme from brand.dev: primary=%s accent=%s font=%s",
            theme["primary"], theme["accent"], theme["font_heading"],
        )
        return theme
    ...
    def _generate_html_report(
        ...
        # Resolve theme (brand.dev colors or Brothers Automate fallback)
        theme = self.resolve_theme(brand)

        # Logo handling - prefer brand.dev URL, fall back to local base64
        logo_html = ""
        if theme["logo_url"]:
            logo_html = f'<img src="{theme["logo_url"]}" alt="{brand.brand_name}" style="max-width:200px;height:auto;margin-bottom:1rem;">'
        else:
            logo_base64 = self._encode_logo()
            if logo_base64:
                logo_html = f'<img src="data:image/png;base64,{logo_base64}" alt="Brothers Automate" style="max-width:200px;height:auto;margin-bottom:1rem;">'

        # Google Fonts URL
        fonts = set()
        fonts.add(theme["font_heading"])
        if theme["font_body"] and theme["font_body"] != theme["font_heading"]:
            fonts.add(theme["font_body"])
        font_url_param = "|".join(f"family={f.replace(' ', '+')}:wght@400;600;700" for f in fonts if f)
        google_fonts_url = f"https://fonts.googleapis.com/css2?{font_url_param}&display=swap"
...
<head>
    ...
    <style>
        :root {{
            --navy: {theme["primary"]};
            --navy-light: {theme["primary_light"]};
            --orange: {theme["accent"]};
            --orange-dark: {theme["accent_dark"]};
            ...
            --header-text: {theme["header_text_color"]};
        }}
        ...
        * {{
        ...
        body {{
            font-family: '{theme["font_body"]}', '{theme["font_heading"]}', -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif;
            line-height: 1.6;
            color: var(--navy);
...
<body>
    <div class="container">
        ...
        <footer>
            <p>Report generated by <strong>{theme["footer_text"]}</strong></p>
